...
\usepackage[margin=2.0cm,paperwidth=180mm,paperheight=1200mm]{geometry}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{amstext}
\usepackage{amsthm}
...
\end{center}
\caption{Beginning at the point plot of $(f(n),g(n))$ in $Z^2$ on the left. The first
dashed \emph{keep-off} lines are shown on the right.}
\label{fig:00}
\end{figure}
...
Thus, for $f(n)$ when $n$ is odd, we need to show that, in algorithm step 3, $f(n)$ is always assigned a larger target value than the previous odd $n$ and also that a smaller value hole is left unused for algorithm step 2 to subsequently fill-in.\\

To do this it will suffice to show that for all $n$, $f(n + 2) > f(n) + 2$.\\

\begin{proof}
\hfill
\begin{enumerate}
\item Assume $n$ is odd.
\item By algorithm 1, step 3, $f(n) = n-g(n)$.
\item Adding 2 to $n$, $f(n+2) = (n+2)-g(n+2)$.
\item By algorithm 1, step 3, $g(n+2) < g(n)$.
\item Therefore, $-g(n+2) > -g(n)$.
\item Adding $(n + 2)$, $(n+2)-g(n+2) > (n+2)-g(n)$.
\item Regrouping, $(n + 2)-g(n+2) > (n-g(n))+2$.
\item Substituting from steps (2) and (3), $f(n+2) > f(n)+2$.
\end{enumerate}
\end{proof}

A similar argument can be applied to $g(n)$.
...
10000001 &17071070 &-7071069 &-0.414213578879\\ \end{tabular}
\end{center}
